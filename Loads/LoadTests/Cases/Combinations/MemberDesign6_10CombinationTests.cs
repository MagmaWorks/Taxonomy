    ...
    public class MemberDesign6_10CombinationTests
    {
        ...
        public void NameAndDefinitionTest()
        {
            ...
            Assert.Equal("LC1: STR/GEO Set B, Eq. 6.10 - Leading Live loads Category B",
                combinations[0].Name);
            Assert.Equal("1.35·(DL + SDL) + 1.5·Q + 1.5·(0.6·W + 0.5·S + 0.6·T)", combinations[0].Definition);

            Assert.Equal("LC2: STR/GEO Set B, Eq. 6.10 - Leading Wind loads",
                combinations[1].Name);
            Assert.Equal("1.35·(DL + SDL) + 1.5·W + 1.5·(0.7·Q + 0.5·S + 0.6·T)", combinations[1].Definition);

            Assert.Equal("LC3: STR/GEO Set B, Eq. 6.10 - Leading Wind loads with unfavourable permanent",
                combinations[2].Name);
            Assert.Equal("DL + SDL + 1.5·W", combinations[2].Definition);

            Assert.Equal("LC4: STR/GEO Set B, Eq. 6.10 - Leading Snow loads",
                combinations[3].Name);
            Assert.Equal("1.35·(DL + SDL) + 1.5·S + 1.5·(0.7·Q + 0.6·W + 0.6·T)", combinations[3].Definition);

            Assert.Equal("LC5: STR/GEO Set B, Eq. 6.10 - Leading Thermal loads",
                combinations[4].Name);
            Assert.Equal("1.35·(DL + SDL) + 1.5·T + 1.5·(0.7·Q + 0.6·W + 0.5·S)", combinations[4].Definition);
        }
